...
## [1.4.3] — 23.07.2026
### Исправления
- Firefox Add-ons требует поле `browser_specific_settings.gecko.data_collection_permissions`
  для всех новых расширений — без него загрузка отклонялась с ошибкой. Добавлено значение
  `authenticationInfo` (расширение передаёт наружу только OAuth-креды входа в Шикимори,
  никакой аналитики/телеметрии на свои сервера нет).
- Этот ключ манифеста поддерживается только с Firefox 140 (и 142 для Android) — поднята
  минимальная версия (`strict_min_version`) с 67 до 140/142 соответственно. Это же убрало
  15 лишних предупреждений валидатора о недоступных на старом Android API
  (`permissions.contains/request/remove`, `browserAction.setBadgeText`).
- Исправлены ссылки в интерфейсе («Исходный код на GitHub», «Подробнее об изменениях»,
  лицензия), которые вели на репозиторий оригинального автора вместо этого форка.
- CI: у GitHub Actions job не хватало прав `contents: write`, из-за чего публикация
  GitHub Release падала с «Resource not accessible by integration».
### Изменено
- Расширение переименовано в «Play Anime Online».
